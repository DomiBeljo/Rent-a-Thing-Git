...
public class DeleteListingHandler implements ListingNotificationHandler {
    ...
    public DeleteListingHandler(EmailService emailService, ListingEmailTemplate template) {
    ...
    public void handle(ListingEvent event) {
        String html = template.buildDeleteHtml(event.getOwnerUsername(), event.getListingId());
        try {
            emailService.sendVerificationEmail(event.getOwnerEmail(), "Rent-a-Thing — Your listing has been removed", html);
